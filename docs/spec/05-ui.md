...
and each row has `Delete`, which requests confirmation.
...
are never positioned manually and edges are never routed manually (P-02). The
...
The text of a `branch` or `check` diamond is its condition as a question: the
...
are not in this menu; slots offer them (U-52).
...
U-50 An `expr` slot's menu opens with one field that accepts the typed
input: digits produce a number, a leading quote produces a text, and
letters filter every item below it. Under the field comes a template row.
For a condition slot (`if`, `while`) the templates are:
...
current values (`"5 is greater than 3" is true, so Yes`); `write` → `i is now
3`; `swap` → `Swapped items 1 and 2 of nums`; `loop` → `Pass 3: i is 3`;
`print` → `Printed "Fizz"`; `call` → `Calling f(2)`; `return` → `f returned
4`, which is the sole narration of a module call (R-11); the end of the
run →
...
by`, and appends `so Yes` / `so No` only when it is a diamond's entire
condition. A `read` of several items → `Read 3 items of nums`; a `loop`
without a variable → `Pass 3`; a list that no variable of the frame
...
(the next unsolved problem of the current plan, otherwise the next row of its
...
U-71 All strings reside in `src/i18n/en.json` (and, from M-10, `ja.json`)
