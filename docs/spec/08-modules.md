...
export type Case = {
  ...
  expect: { returns: Data; args: Data[] }; // the value returned, and the arguments afterwards
};
```
...
modules first in name order; `New module…` (asks the name, L-01),
...
asks; afterwards those calls are `E_UNKNOWN_CALL` (U-37).
...
function. The function's arguments go with it: from a run, the arguments
of that call as they were at its `call` event (R-11 Seek); in build mode,
the arguments that are literals; otherwise none. They travel with the
module and function names under `localStorage` key `algoflow:handoff`,
...
case holding that run's arguments, returned value, and arguments
afterwards; a case can be deleted.
...
arguments afterwards, compared by C-10, with `▶ Watch this case`.

D-21 When `Move to module…` (D-09) puts a function into a learner module
...
`get(name)`, `put(module)`, and `remove(name)`; nothing else touches the
key.

D-22 The store re-reads `algoflow:modules` on the window's `storage`
event, so an edit in one browser tab reaches the others: open programs are
validated again. A run in progress keeps the modules it started with
(R-01); the next Run uses the changed ones.
...
`heap_pop(heap)`, which takes the root, moves the last item to the root,
