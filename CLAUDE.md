...
and the kickoff prompts `private/PROMPTS.md` are ignored by git: read them,
update them, never commit them. `docs/` is tracked: a spec change lands in
its own commit before the code that implements it.
