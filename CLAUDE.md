...
- `docs/spec/` says what must be; the code says what is; the open GitHub
  milestone, its issues, and the open pull requests say what is next. There
...
- `private/PROMPTS.md` (ignored by git) holds the kickoff prompts the user
...
the moment `pnpm lint && pnpm test` is green: a node plus its i18n keys and
both tests, a challenge, a view. Land cross-cutting type, event, or spec
changes as their own commit before the code that uses them.
...
Keep pull requests under ~600 changed lines; split a milestone into
reviewable pieces. `main` takes rebase merges only, after CI. The body follows
...
- oxlint: store actions must be function properties (`unbound-method`);
  `jsx-a11y` refuses mouse and click handlers on `li`/`tr`/`ol` (use one
  delegated listener with `data-*`); `react/set-state-in-effect` refuses
  `setState` inside effects (adjust state during render); `no-base-to-string`
  refuses `String(unknown)`; `no-await-in-loop` is a warning the batch loops
  accept.
- Hooks: a PostToolUse hook runs oxfmt on every written file under `src/`
  and `challenges/` but not `scripts/` (run `pnpm exec oxfmt scripts` by
  hand; `pnpm lint` does not check formatting); re-read a file before a
...
- Multi-line edit scripts: write them to the scratchpad and run the file;
  heredocs passed to the Bash tool fail on some quoting.
- `pnpm check` needs `python3` on PATH; e2e runs against `dist/`, so build
  first. A chart can be checked visually without a screen: write `layout()`'s
  result as SVG from a temporary Vitest file and screenshot it with Playwright.
